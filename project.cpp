...
vector<int> the_arr(int All_Goods, int num, int Number_Trades)
{
	...
	for (int i = 1; i <= Number_Trades; i++) //это происходит количество-сделок-раз(перебор всех сделок)
	{
		...
		vector<vector<int>> Matrix_local = matrix_creator(True_arr2, num, Trade_Goods); // матрица сочетаний данной строки
...
int main()
{
	...
	while (!file.eof())
	{
		...
		if (chr <'0'&&chr>'9')
		{
			cout << "there are wrong symbols in the file.the information cannot be read properly"; //проверка на только цифры
			return 0;
		}
	}
	file.close();
