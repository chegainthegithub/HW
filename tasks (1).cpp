...
char *rememory(char *text, unsigned n)
{
	char *temp = new char[n];
	strncpy(temp, text, n-1);
	delete[] text;
	return temp;
}
...
char *strnstr(char* base, char* find, unsigned n)//равенство первых n символов строки
{
	for (int i = 0; i < n; i++)
	{
		if (base[i] != find[i])
			...
			else
			{
				if (tempsize == max) //одинаково большое слово
				{
					...
					if (!(i < words))//увеличиваем кол-во слов
					{
						...
						{
							strcmp(temp2[j], word[j]);//////////////////////////////////////вот это не хочет копировать
						}
						strncmp(temp2[words-1], temp, max);
						if (words-1 != 1)//на удаление 1 элемента
							for (int j = 0; j < words - 1; j++)
							{
								delete[] word[j];
							}
...
void charcopy(char *here, char* what, unsigned n)//заменяет нерабочую strncpy
{
	for (int i = 0; i < n; i++)
	{
		here[i] = what[i];
